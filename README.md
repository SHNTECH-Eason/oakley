...
**天天會出來稱讚他。** 完成時天天從右下角彈出來揮手、眨眼，帶一個對話泡泡說出稱讚語（有注音），約兩秒後自己退場。給特別獎勵時會放大一號、停久一點。

角色圖是 `pointer-events: none`，不會擋到任何按鈕；系統開了「減少動態效果」時停在第一格不播放。

### 動畫素材怎麼來的

`assets/skye-cheer.webp` 是一張 **3600×200 的精靈圖**，橫排 18 格、每格 200×200，用 CSS `steps(18)` 播放，不需要影片解碼也不依賴任何函式庫。
...
3. 挑出動作最好的 1.5 秒（揮手＋眨眼），以 12fps 取 18 格
